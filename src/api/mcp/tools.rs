...
pub async fn handle_tool_call(name: &str, args: Value, auth: &AuthContext) -> Result<Value, String> {
    match name {
        "query_database" => {
            let db_name = args.get("db_name").and_then(|v| v.as_str()).ok_or("Missing db_name")?;
