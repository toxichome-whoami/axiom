...
async fn metrics_endpoint(Query(params): Query<HashMap<String, String>>) -> Response {
    ...
    output.push_str(&format_gauge(
        "axiom_request_duration_avg_ms",
        avg_duration,
        "Avg latency",
        "gauge",
    ));
    output.push_str(&format_gauge(
        "axiom_request_duration_p99_ms",
        p99,
        "P99 latency",
        "gauge",
    ));
    ...
    output.push_str(&format_metric(
        "axiom_webhook_delivered_total",
        wd,
        "",
        "counter",
    ));
    output.push_str(&format_metric(
        "axiom_webhook_failed_total",
        wf,
        "",
        "counter",
    ));
