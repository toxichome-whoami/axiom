...
pub fn get_project(headers: &HeaderMap) -> Result<(String, Value), AxiomError> {
    let api_key = headers
        ...
        .ok_or_else(|| AxiomError::new("AUTH_MISSING_HEADER", "Missing API Key", StatusCode::UNAUTHORIZED))?;

    let decoded = base64::engine::general_purpose::STANDARD.decode(api_key)
        .ok()
        .and_then(|b| String::from_utf8(b).ok())
        .ok_or_else(|| AxiomError::new("AUTH_INVALID_FORMAT", "Invalid API Key format", StatusCode::UNAUTHORIZED))?;

    let (key_name, key_secret) = decoded.split_once(':')
        .ok_or_else(|| AxiomError::new("AUTH_INVALID_FORMAT", "Invalid API Key format", StatusCode::UNAUTHORIZED))?;

    let config = ConfigManager::get();
    if !config.features.auth {
        return Err(AxiomError::new("AUTH_INSUFFICIENT_MODE", "Auth module is disabled", StatusCode::FORBIDDEN));
    }

    let key_cfg = config.api_key.get(key_name)
        .ok_or_else(|| AxiomError::new("AUTH_INVALID_SECRET", "Invalid API Key", StatusCode::UNAUTHORIZED))?;

    if key_cfg.secret != key_secret {
        return Err(AxiomError::new("AUTH_INVALID_SECRET", "Invalid API Key secret", StatusCode::UNAUTHORIZED));
    }

    let project_cfg = config.auth.project.get(key_name)
        .cloned()
        .map(|v| serde_json::to_value(v).unwrap_or(json!({})))
        .ok_or_else(|| AxiomError::new("AUTH_PROJECT_NOT_CONFIGURED", "Auth not configured for this key", StatusCode::FORBIDDEN))?;
...
fn get_bearer(headers: &HeaderMap) -> Result<String, AxiomError> {
    headers
        ...
        .map(|s| s.to_string())
        .ok_or_else(|| AxiomError::new("AUTH_MISSING_HEADER", "Missing Bearer token", StatusCode::UNAUTHORIZED))
}
...
async fn build_auth_response(
    ...
    let access_token = create_access_token(
        project_id, &uid, &email, email_verified, is_anonymous, false,
        access_ttl, HashMap::new(),
    ...
    issue_refresh_token(pool, &uid, &token_hash, &family_id, &expires_at, Some(ip), Some(ua)).await?;
...
pub async fn handler_signup(headers: HeaderMap, Json(body): Json<SignupRequest>) -> Result<Json<Value>, AxiomError> {
    ...
    if !updates.is_empty() { update_user(&pool, &uid, &updates).await?; }

    log_audit(&pool, "signup", Some(&uid), Some(&get_ip(&headers)), Some(&get_ua(&headers)), None).await?;
    ...
    let resp = build_auth_response(&pool, &config, &project_id, &user_row, &get_ip(&headers), &get_ua(&headers)).await?;
    Ok(Json(resp))
}

pub async fn handler_login(headers: HeaderMap, Json(body): Json<LoginRequest>) -> Result<Json<Value>, AxiomError> {
    let (project_id, config) = get_project(&headers)?;
    let pool = get_pool(&project_id).await?;
    let ip = get_ip(&headers);
    let ua = get_ua(&headers);

    let row = get_user_by_email(&pool, &body.email).await
        .ok_or_else(|| AxiomError::new("AUTH_INVALID_CREDENTIALS", "Invalid credentials", StatusCode::UNAUTHORIZED))?;

    if row["disabled"].as_i64().unwrap_or(0) != 0 {
        return Err(AxiomError::new("AUTH_ACCOUNT_DISABLED", "Account is disabled", StatusCode::FORBIDDEN));
    }

    let hash = row["password_hash"].as_str().unwrap_or_default();
    if hash.is_empty() || !verify_password(hash, &body.password) {
        log_audit(&pool, "login_failed", row["uid"].as_str(), Some(&ip), Some(&ua), None).await?;
        return Err(AxiomError::new("AUTH_INVALID_CREDENTIALS", "Invalid email or password", StatusCode::UNAUTHORIZED));
    }
    ...
    if row["totp_enabled"].as_i64().unwrap_or(0) != 0 {
        ...
        let mfa_token = create_access_token(&project_id, uid, email, false, false, false, 300,
            [("mfa_pending".to_string(), json!(true))].into_iter().collect()
        ).await.map_err(|e| AxiomError::new("AUTH_TOKEN_ERROR", &e, StatusCode::INTERNAL_SERVER_ERROR))?;
        return Ok(Json(json!({ "mfa_required": true, "mfa_token": mfa_token })));
    }

    log_audit(&pool, "login_success", row["uid"].as_str(), Some(&ip), Some(&ua), None).await?;
    let resp = build_auth_response(&pool, &config, &project_id, &row, &ip, &ua).await?;
    Ok(Json(resp))
}

pub async fn handler_refresh(headers: HeaderMap, Json(body): Json<RefreshRequest>) -> Result<Json<Value>, AxiomError> {
    let (project_id, config) = get_project(&headers)?;
    let pool = get_pool(&project_id).await?;
    let ip = get_ip(&headers);
    let ua = get_ua(&headers);
    ...
    let token_row = get_refresh_token(&pool, &token_hash).await
        .ok_or_else(|| AxiomError::new("AUTH_TOKEN_INVALID", "Invalid refresh token", StatusCode::UNAUTHORIZED))?;
    ...
    if token_row["revoked"].as_i64().unwrap_or(0) != 0 {
        ...
        return Err(AxiomError::new("AUTH_TOKEN_STOLEN", "Token reuse detected. All sessions revoked.", StatusCode::UNAUTHORIZED));
    }

    let expires_at_str = token_row["expires_at"].as_str().unwrap_or_default();
    let expires_at = chrono::DateTime::parse_from_rfc3339(expires_at_str)
        .map_err(|_| AxiomError::new("AUTH_TOKEN_INVALID", "Invalid token expiry", StatusCode::UNAUTHORIZED))?;
    if expires_at < Utc::now() {
        return Err(AxiomError::new("AUTH_TOKEN_EXPIRED", "Refresh token expired", StatusCode::UNAUTHORIZED));
    }

    let uid = token_row["uid"].as_str().unwrap_or_default().to_string();
    let user_row = get_user_by_uid(&pool, &uid).await
        .ok_or_else(|| AxiomError::new("AUTH_ACCOUNT_DISABLED", "Account not found", StatusCode::FORBIDDEN))?;

    if user_row["disabled"].as_i64().unwrap_or(0) != 0 {
        return Err(AxiomError::new("AUTH_ACCOUNT_DISABLED", "Account disabled", StatusCode::FORBIDDEN));
    }
...
pub async fn handler_logout(headers: HeaderMap, Json(body): Json<LogoutRequest>) -> Result<Json<Value>, AxiomError> {
    ...
    sqlx::query("UPDATE refresh_tokens SET revoked = 1 WHERE token_hash = ? AND uid = ?")
        .bind(&token_hash).bind(&claims.sub)
        .execute(&pool).await.ok();

    log_audit(&pool, "logout", Some(&claims.sub), Some(&get_ip(&headers)), Some(&get_ua(&headers)), None).await?;
...
pub async fn handler_get_me(headers: HeaderMap) -> Result<Json<Value>, AxiomError> {
    ...
    let pool = get_pool(&project_id).await?;
    let user = get_user_by_uid(&pool, &claims.sub).await
...
pub async fn handler_get_sessions(headers: HeaderMap) -> Result<Json<Value>, AxiomError> {
    ...
    let sessions: Vec<Value> = rows.iter().map(|r| {
        ...
        for col in r.columns() {
            let v: Value = r.try_get::<Option<String>, _>(col.name())
                .ok()
                .flatten()
                .map(Value::String)
...
pub async fn handler_revoke_all_sessions(headers: HeaderMap) -> Result<Json<Value>, AxiomError> {
    ...
    log_audit(&pool, "session_revoked_all", Some(&claims.sub), Some(&get_ip(&headers)), Some(&get_ua(&headers)), None).await?;
    Ok(Json(json!({ "status": "ok" })))
}

pub async fn handler_verify_email(headers: HeaderMap, Json(body): Json<VerifyEmailRequest>) -> Result<Json<Value>, AxiomError> {
    let (project_id, _) = get_project(&headers)?;
    let pool = get_pool(&project_id).await?;
    ...
    if used != 0 {
        return Err(AxiomError::new("AUTH_TOKEN_INVALID", "Token already used", StatusCode::BAD_REQUEST));
    }
    ...
    sqlx::query("UPDATE auth_tokens SET used = 1 WHERE token_hash = ?")
        .bind(&token_hash).execute(&pool).await.ok();

    Ok(Json(json!({ "status": "ok" })))
}

pub async fn handler_forgot_password(headers: HeaderMap, Json(body): Json<ForgotPasswordRequest>) -> Result<Json<Value>, AxiomError> {
    let (project_id, config) = get_project(&headers)?;
    let pool = get_pool(&project_id).await?;
...
pub async fn handler_reset_password(headers: HeaderMap, Json(body): Json<ResetPasswordRequest>) -> Result<Json<Value>, AxiomError> {
    let (project_id, config) = get_project(&headers)?;
    let pool = get_pool(&project_id).await?;
    let token_hash = sha256_hex(&body.token);
    ...
    sqlx::query("UPDATE users SET password_hash = ?, updated_at = ? WHERE uid = ?")
        .bind(&pw_hash).bind(utc_now_iso()).bind(&uid).execute(&pool).await.ok();
    sqlx::query("UPDATE auth_tokens SET used = 1 WHERE token_hash = ?")
        .bind(&token_hash).execute(&pool).await.ok();
...
pub async fn admin_list_users(headers: HeaderMap, Query(params): Query<HashMap<String, String>>) -> Result<Json<Value>, AxiomError> {
    let bearer = get_bearer(&headers)?;
    let (project_id, _) = get_project(&headers)?;
    verify_access_token(&bearer, &project_id).await
        .map_err(|e| AxiomError::new("AUTH_TOKEN_INVALID", &e, StatusCode::UNAUTHORIZED))?;
    ...
    let users: Vec<Value> = rows.iter().map(|r| {
        ...
        for col in r.columns() {
            let v: Value = r.try_get::<Option<String>, _>(col.name())
                .ok().flatten().map(Value::String)
                .or_else(|| r.try_get::<Option<i64>, _>(col.name()).ok().flatten().map(|n| json!(n)))
                .unwrap_or(Value::Null);
            map.insert(col.name().to_string(), v);
        }
        Value::Object(map)
...
pub async fn admin_delete_user(headers: HeaderMap, Path(uid): Path<String>) -> Result<Json<Value>, AxiomError> {
    let bearer = get_bearer(&headers)?;
    let (project_id, _) = get_project(&headers)?;
    verify_access_token(&bearer, &project_id).await
        .map_err(|e| AxiomError::new("AUTH_TOKEN_INVALID", &e, StatusCode::UNAUTHORIZED))?;
...
pub async fn admin_audit_log(headers: HeaderMap, Query(params): Query<HashMap<String, String>>) -> Result<Json<Value>, AxiomError> {
    let bearer = get_bearer(&headers)?;
    let (project_id, _) = get_project(&headers)?;
    verify_access_token(&bearer, &project_id).await
        .map_err(|e| AxiomError::new("AUTH_TOKEN_INVALID", &e, StatusCode::UNAUTHORIZED))?;
