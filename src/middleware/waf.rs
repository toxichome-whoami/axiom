...
pub async fn waf_middleware(
    ...
    if let Some(cl) = req.headers().get("content-length") {
        if let Ok(cl_str) = cl.to_str() {
            if let Ok(size) = cl_str.parse::<u64>() {
                if size > body_limit {
                    return Err(AxiomError::new("WAF_BODY_TOO_LARGE", "Request body too large.", axum::http::StatusCode::PAYLOAD_TOO_LARGE));
                }
            }
        }
    }
    ...
    if path.len() + query.len() > 2048 {
        return Err(AxiomError::new("WAF_URI_TOO_LONG", "URI exceeds 2048 characters", axum::http::StatusCode::URI_TOO_LONG));
    }
    ...
    if path.contains("..") || path.contains("%") || query.contains("..") || query.contains("%") {
        ...
        if path.contains("../") || path.contains("..\\") || path.contains("%2e%2e%2f") {
            return Err(AxiomError::new("WAF_PATH_TRAVERSAL", "Path traversal attempt detected", axum::http::StatusCode::BAD_REQUEST));
        }
    }

    if path.contains("\0") || query.contains("\0") {
        return Err(AxiomError::new("WAF_NULL_BYTE", "Null byte detected", axum::http::StatusCode::BAD_REQUEST));
    }

    if query.matches('&').count() > 50 {
        return Err(AxiomError::new("WAF_TOO_MANY_PARAMS", "Too many query parameters", axum::http::StatusCode::BAD_REQUEST));
    }
