...
## 8. Presigned URLs & Migrations Authorization

**Presigned URLs** allow time-limited, unauthenticated access to storage files without exposing API keys. The security model is:

- A signed URL is generated by a backend holding a valid API key (`POST /api/v1/fs/{alias}/presign`).
- The signature is computed as `HMAC-SHA256(secret, "METHOD:alias:path:expires")` using the API key's `secret` as the signing key.
- The signature and expiry timestamp are embedded as query parameters in the returned URL.
- The `auth_middleware` validates the signature and expiry server-side on every request. Expired URLs return `401 AUTH_EXPIRED`. Invalid signatures silently fall through to standard key authentication (no information leak about which part failed).
- No API key header is needed to use a presigned URL — the signature alone authorizes the request for the duration of its TTL.

**Database Migrations** (`POST /api/v1/db/{alias}/migrations`) are protected by an explicit `full_admin = true` authorization check enforced in the handler layer before any SQL is executed. Keys without `full_admin` receive `403 Forbidden`.
