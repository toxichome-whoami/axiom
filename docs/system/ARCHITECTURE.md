...
- **WebSocket (`/api/v1/ws`)** *(optional)*: A Starlette-native WebSocket endpoint using zero additional dependencies. The `ConnectionManager` maintains an in-process `{client_id → WebSocket}` registry. The `EventBus` is called as a fire-and-forget side channel from `emit_event()` — adding zero latency to the primary REST path. Clients authenticate via first JSON message, then subscribe to DB/FS/metrics topics scoped to their API key permissions.
