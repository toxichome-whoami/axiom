...
- **Storage (`/api/fs`)**: Directory listing requests use a single `tokio::task::spawn_blocking` call wrapping `std::fs::read_dir` — this batches all metadata fetches into one blocking task instead of spawning individual async operations per entry. Implements active `CircuitBreaker` integrations to block bandwidth saturation. All large file uploads via the `ChunkedUploadManager` use direct socket-to-disk 1MB streams leveraging native sparse-file offset allocation with on-the-fly threaded cryptographic hashing, rendering the gateway fully immune to Out-of-Memory (OOM) crashes during massive concurrent uploads.
