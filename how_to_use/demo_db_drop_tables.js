...
const B64_AUTH = Buffer.from(`${CONFIG.keyName}:${CONFIG.secret}`).toString(
    "base64",
);
...
async function sendRequest(apiPath, method, payload = null) {
    ...
    return new Promise((resolve, reject) => {
        const req = protocolClient.request(options, (res) => {
            ...
            res.on("end", () =>
                processFinalResponse(res, chunkedData, resolve, reject),
            );
        });
...
function processFinalResponse(res, rawData, resolve, reject) {
    ...
    try {
        ...
        reject(
            new Error(
                `JSON Parse Error: ${error.message}. Content: ${rawData}`,
            ),
        );
    }
}
