...
const AUTH_TOKEN_B64 = Buffer.from(
...
async function apiCall(endpoint, method, payload = null, customHeaders = {}) {
    ...
    return new Promise((resolve, reject) => {
        const req = networkClient.request(url, requestOptions, (res) => {
            ...
            res.on("end", () =>
                processApiResponse(res, buffer, resolve, reject),
            );
        });
...
function processApiResponse(res, rawBody, resolve, reject) {
    try {
        ...
        reject(
            new Error(`[${res.statusCode}] Could not parse JSON: ${rawBody}`),
        );
    }
}
...
async function runChunkedUploadFlow(alias, localFilePath, targetRelativePath) {
    if (!fs.existsSync(localFilePath))
        return console.error(`❌ Local file missing: ${localFilePath}`);
    ...
    console.log(
        `🚀 [START] Chunked Transfer: '${fileName}' (${(totalBytes / 1024 / 1024).toFixed(2)} MB)`,
    );

    try {
        const initResponse = await startUploadSession(
            alias,
            fileName,
            destination,
            totalBytes,
        ...
        for (let i = 0; i < totalChunks; i++) {
            ...
            fs.readSync(
                ...
                0,
                buffer.length,
                currentChunkInfo.offset,
            );

            const progress = ((i / totalChunks) * 100).toFixed(1);
            process.stdout.write(
                `📤 Transmitting Chunk ${i + 1}/${totalChunks} (${progress}%)... `,
            );
...
function main() {
    ...
    if (!localPathArg) {
        return console.log(
            "Usage: node upload.js <alias> <local_path> [remote_path]",
        );
    }
