        ...
        const req = protocol.request(url, options, res => {
            ...
            res.on('end', () => {
                if (res.statusCode === 404) {
                    return reject(new Error("404 Not Found — Is `features.graphql = true` in config.toml?"));
                }
                try { 
                    resolve({ status: res.statusCode, data: JSON.parse(raw) }); 
                } catch { 
                    reject(new Error(`[${res.statusCode}] Failed to parse response: ${raw}`)); 
                }
            });
        });
...
async function runTests() {
    ...
    try {
        ...
        if (dbRes.status === 200 && dbRes.data.data) {
            console.log('✅ Success! Available databases:', dbRes.data.data.databases.join(', '));
        } else {
        ...
        if (execRes.status === 200 && execRes.data.data && execRes.data.data.execute) {
            ...
            if (execRes.data.extensions) {
                console.log(`   Timing:  ${execRes.data.extensions.duration_ms}ms execution`);
            }
        } else {
